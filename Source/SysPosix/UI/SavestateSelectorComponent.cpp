...
#include "SysPosix/UI/UICommand.h"
#include "SysPosix/UI/UIContext.h"
#include "SysPosix/UI/UIElement.h"
#include "SysPosix/UI/UIScreen.h"
#include "SysPosix/UI/PSPMenu.h"
#include "SysPosix/UI/SavestateSelectorComponent.h"
...
void ISavestateSelectorComponent::LoadSlots(){
    ...
    for (u32 i = 0; i < NUM_SAVESTATE_SLOTS; ++i) {
        ...
        if (!rom_id.Empty() && CRomSettingsDB::Get()->GetSettings(rom_id, &settings)) {
            stat(filename_ss, &file_stat);
            sprintf(date_string, "%02d/%02d/%d %02d:%02d:%02d", localtime(&file_stat.st_ctime)->tm_mon + 1,
                    localtime(&file_stat.st_ctime)->tm_mday, localtime(&file_stat.st_ctime)->tm_year + 1900,
                    localtime(&file_stat.st_ctime)->tm_hour, localtime(&file_stat.st_ctime)->tm_min,
                    localtime(&file_stat.st_ctime)->tm_sec);
            str << date_string;
            mSlotEmpty[i] = false;
        } else {
...
void	ISavestateSelectorComponent::Update( float elapsed_time, const v2 & stick, u32 old_buttons, u32 new_buttons )
{
	...
	if(old_buttons != new_buttons)
	{
	  if(mAccessType == AT_LOADING && deleteButtonTriggered)
		{
		  if( new_buttons & SDL_CONTROLLER_BUTTON_Y )
			{
				deleteSlot(mElements.GetSelectedIndex());
			}
		}

		if( new_buttons & SDL_CONTROLLER_BUTTON_DPAD_UP )
		{
			mElements.SelectPrevious();
			if(mAccessType == AT_LOADING)
			    deleteButtonTriggered=false;
		}
		if( new_buttons & SDL_CONTROLLER_BUTTON_DPAD_DOWN )
		{
		...
		if( element != NULL )
		{
			if( new_buttons & SDL_CONTROLLER_BUTTON_DPAD_LEFT )
			{
				element->OnPrevious();
			}
			if( new_buttons & SDL_CONTROLLER_BUTTON_DPAD_RIGHT )
			{
				element->OnNext();
			}
			if( new_buttons & (SDL_CONTROLLER_BUTTON_A|SDL_CONTROLLER_BUTTON_START) )
			{
				// Commit settings
				element->OnSelected();
			}
			if( new_buttons & SDL_CONTROLLER_BUTTON_X)
			{
				//delete savestate
			...
			if( new_buttons & (SDL_CONTROLLER_BUTTON_B|SDL_CONTROLLER_BUTTON_BACK) )
			{
				// Discard settings
				deleteButtonTriggered=false;
