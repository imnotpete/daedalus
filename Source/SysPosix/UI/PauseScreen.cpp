...
void	IPauseScreen::Update( float elapsed_time, const v2 & stick, u32 old_buttons, u32 new_buttons )
{
	...
	if(!(new_buttons & SDL_CONTROLLER_BUTTON_GUIDE) && button_released)
	{
	...
	{
		if(new_buttons & SDL_CONTROLLER_BUTTON_LEFTSHOULDER)
		{
			mCurrentOption = GetPreviousValidOption();
			new_buttons &= ~SDL_CONTROLLER_BUTTON_LEFTSHOULDER;
		}
		if(new_buttons & SDL_CONTROLLER_BUTTON_LEFTSHOULDER)
		{
		...
		if(new_buttons & SDL_CONTROLLER_BUTTON_GUIDE)
		{
			...
			new_buttons &= ~SDL_CONTROLLER_BUTTON_GUIDE;
		}
	}
