...
namespace
{
	...
	public:
		...
		{
			Translate_Read( gGlobalPreferences.Language, DAEDALUS_POSIX_PATH("Languages/") );
		}
		virtual const char *	GetSettingName() const
		{
	...
	public:
		...
		virtual	void			OnSelected()
		{
			...
			{
				/*
				DeleteRecursive(DAEDALUS_POSIX_PATH("SaveGames/Cache"),".hle");
				ThreadSleepMs(1000);	//safety wait for s

				if(gShowDialog.Render( mpContext,"Reset settings?", false) )
				{
					remove(DAEDALUS_POSIX_PATH("preferences.ini"));
					remove(DAEDALUS_POSIX_PATH("rom.db"));
					ThreadSleepMs(1000);	//safety wait for s
