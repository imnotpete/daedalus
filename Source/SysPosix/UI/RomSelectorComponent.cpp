...
#include "SysPosix/UI/RomSelectorComponent.h"
...
void	IRomSelectorComponent::Update( float elapsed_time, const v2 & stick, u32 old_buttons, u32 new_buttons )
{
	...
	mDisplayFilenames = (new_buttons & SDL_CONTROLLER_BUTTON_Y) != 0;

	//mDisplayInfo = (new_buttons & SDL_CONTROLLER_BUTTON_X) != 0;
	...
	if(old_buttons != new_buttons)
	{
		...
		if(new_buttons & SDL_CONTROLLER_BUTTON_GUIDE)
		{
			//sceKernelExitGame();
		}
#endif
		if(new_buttons & SDL_CONTROLLER_BUTTON_A && mRomDelete)	// DONT CHANGE ORDER
		{
			remove( mSelectedRom.c_str() );
			mRomDelete = false;
			UpdateROMList();
		}
		else if((new_buttons & SDL_CONTROLLER_BUTTON_START) ||
			(new_buttons & SDL_CONTROLLER_BUTTON_A))
		{
			if(mCurrentSelection < mRomsList.size())
			{
		...
		if(new_buttons & SDL_CONTROLLER_BUTTON_BACK)
		{
			if(mCurrentSelection < mRomsList.size())
			{
				mSelectedRom = mRomsList[ mCurrentSelection ]->mFilename;
